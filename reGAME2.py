...
class Bullet(pygame.sprite.Sprite):
    ...
    def moving(self, force, enemy, enemy_health):
        ...
        if self.rect.x > 1000:
            self.ready = True
        if enemy.is_alive:
            if (self.rect.x == enemy.rect.x - 11) and self.rect.y in range(enemy.hitbox[0], enemy.hitbox[1]):
                hit_sound.play()
                self.hit = True
...
class Enemy(pygame.sprite.Sprite):
    def __init__(self, x, y, filename):
        ...
        self.down = False
        self.hp = 3
        self.hitbox = [self.rect.y - 30, self.rect.y + 160]
...
marsik_health = Enemy_health(890, 50, list_of_enemy_health)
...
while not end_game:
    ...
    if level1:
        ...
        if len(list_of_bullet) >= 1:
            ...
            #print(list_of_bullet[number_of_bullet - 1].rect.x)
            if list_of_bullet[number_of_bullet - 1].ready:
                Ready_for_attac = True
        if keys[pygame.K_UP]:
            if Ready_for_attac:
                bullet = Bullet(344, coming_car.rect.y + 90, "D:\SPRITES PHOTO\q2bullet.png")
                now_bullet = Bullet
